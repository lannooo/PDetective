...
class Model(nn.Module):
    def __init__(self):
        ...
        ####
        self.ssl_model = Frontend.load_front_layer("wav2vec2", "facebook/wav2vec2-xls-r-300m", 1024)
    ...
    def forward(self, x):
        ...
        x = self.ssl_proj(x_ssl_feat) #(bs,frame_number,feat_out_dim)
        
        # post-processing on front-end features
        x = x.transpose(1, 2)        # (B, feat_dim, n_frame)
        x = x.unsqueeze(dim=1)       # (B, 1, feat_dim, n_frame)
        x = F.max_pool2d(x, (3, 3))  # (B, 1, downsampled_dim, downsampled_frame)
        x = self.first_bn(x)
        x = self.selu(x)

        # Res-based encoder
        x = self.encoder(x)          # (B, 64, dim, frame)
        x = self.first_bn1(x)        
        x = self.selu(x)
        
        w = self.attention(x)        # ? self attention-based aggregation 
...
if __name__ == "__main__":
    model = Model()
    x = torch.randn(3, int(16000*4+80))
    y = model(x)
    print(y.shape)
